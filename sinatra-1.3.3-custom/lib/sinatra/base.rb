...
module Sinatra
  ...
  private
    ...
    def route_eval
      filter! :before_when_route_is_taken
      throw :halt, yield
    end
    ...
    class << self
      ...
      def reset!
        @conditions     = []
        @routes         = {}
        @filters        = {:before_when_route_is_taken=> [], :before => [], :after => []}
        @errors         = {}
        @middleware     = []
        @prototype      = nil
        @extensions     = []
      ...
      # Defines a before filter; runs before requests that are
      # taken within the same context as route handlers and may
      # access/modify the request and response.
      def before_when_route_is_taken(path = nil, options = {}, &block)
        add_filter(:before_when_route_is_taken, path, options, &block)
      end
  ...
  module Delegator #:nodoc:
    ...
    delegate :get, :patch, :put, :post, :delete, :head, :options, :template, :layout,
             :before, :before_when_route_is_taken, :after, :error, :not_found, :configure, :set, :mime_type,
